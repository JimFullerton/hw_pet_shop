myFunctions = {
  ...
  sellPetToCustomer(shop, pet, cust) {
    if ((pet) && (myFunctions.customerCanAffordPet(cust, pet))) {
      myFunctions.addPetToCustomer(cust, pet);
      myFunctions.increasePetsSold(shop, 1);
      myFunctions.removeCustomerCash(cust, pet.price);
